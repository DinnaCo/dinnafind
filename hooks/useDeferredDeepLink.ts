...
import type { DeferredLinkData } from '@/models/deep-link';
      ...
      try {
        ...
        if (deferredLinkData) {
          ...
          if (data.timestamp > oneHourAgo && !data.processed) {
            logger.info('Found deferred deep link:', data.url);
        ...
        logger.error('Error checking deferred link:', error);
      }
    };
...
export async function simulateDeferredDeepLink(url: string) {
  ...
  logger.info('Deferred deep link stored:', url);
}
...
export async function clearDeferredDeepLink() {
  await AsyncStorage.removeItem(DEFERRED_LINK_KEY);
  logger.info('Deferred deep link cleared');
}
