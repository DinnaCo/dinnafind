...
export const useGeolocation = (): GeolocationHook => {
  ...
  const requestLocation = useCallback(async () => {
    try {
      logger.info('🗺️ useGeolocation: Starting location request');
      setLoading(true);
      setError(null);

      // Request permissions
      const { status } = await Location.requestForegroundPermissionsAsync();
      logger.info('🗺️ useGeolocation: Permission request result:', status);
      ...
      if (status !== 'granted') {
        ...
        logger.info('🗺️ useGeolocation:', errorMsg);
        setError(errorMsg);
        setLoading(false);
        return;
      }
      ...
      logger.info('🗺️ useGeolocation: Getting current position...');
      const location = await Location.getCurrentPositionAsync({
        accuracy: Location.Accuracy.Balanced,
      });

      logger.info('🗺️ useGeolocation: Location received:', location.coords);
      ...
      logger.info('🗺️ useGeolocation: Location updated in Redux:', newCoordinates);
    } catch (error: unknown) {
      const errorMsg = (error as Error).message || 'Failed to get location';
      logger.error('🗺️ useGeolocation: Error getting location:', error);
      setError(errorMsg);
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    const checkPermission = async () => {
      try {
        ...
        logger.info('🗺️ useGeolocation: Current permission status:', status);
        dispatch(setLocationPermission(status === 'granted'));
        setPermissionChecked(true);

        // Only request location if permission is granted and we don't have coordinates
        if (status === 'granted' && !coordinates) {
          logger.info('🗺️ useGeolocation: Permission granted, requesting location');
          requestLocation();
        }
      } catch (err) {
