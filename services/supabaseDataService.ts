...
export class SupabaseDataService {
  ...
  static async loadUserProfile(userId: string): Promise<UserProfile | null> {
    logger.info('[SupabaseDataService] Loading user profile for user ID:', userId);
    try {
      ...
      if (error) {
        logger.error('[SupabaseDataService] Error loading user profile:', error);
        return null;
      }

      if (!data) {
        logger.info('[SupabaseDataService] No user profile found for user ID:', userId);
        return null;
      }
      ...
      logger.info('[SupabaseDataService] User profile loaded successfully:', userProfile);
      return userProfile;
    } catch (error) {
      logger.error('[SupabaseDataService] Failed to load user profile:', error);
      return null;
    }
  }
  ...
  static async loadBucketListItems(userId: string): Promise<BucketListItem[]> {
    logger.info('[SupabaseDataService] Loading bucket list items for user ID:', userId);
    try {
      const { data, error } = await supabase
      ...
      if (error) {
        logger.error('[SupabaseDataService] Error loading bucket list items:', error);
        return [];
      }

      if (!data) {
        logger.info('[SupabaseDataService] No bucket list items found for user ID:', userId);
        return [];
      }
      ...
      logger.info(`[SupabaseDataService] Loaded ${bucketListItems.length} bucket list items`);
      return bucketListItems;
    } catch (error) {
      logger.error('[SupabaseDataService] Failed to load bucket list items:', error);
      return [];
    }
  }
  ...
  static async loadUserPreferences(userId: string): Promise<Record<string, any>> {
    logger.info('[SupabaseDataService] Loading user preferences for user ID:', userId);
    try {
      const { data, error } = await supabase
        .from('user_profiles')
        .select('preferences')
      ...
      if (error) {
        logger.error('[SupabaseDataService] Error loading user preferences:', error);
        return {};
      }

      const preferences = data?.preferences || {};
      logger.info('[SupabaseDataService] User preferences loaded:', preferences);
      return preferences;
    } catch (error) {
      logger.error('[SupabaseDataService] Failed to load user preferences:', error);
      return {};
    }
  }
  ...
  static async upsertUserProfile(profile: Partial<UserProfile>): Promise<boolean> {
    logger.info('[SupabaseDataService] Starting upsert user profile for user ID:', profile.id);
    try {
      ...
      if (error) {
        logger.error('[SupabaseDataService] Error upserting user profile:', error);
        return false;
      }

      logger.info('[SupabaseDataService] User profile upserted successfully');
      return true;
    } catch (error) {
      logger.error('[SupabaseDataService] Failed to upsert user profile:', error);
      return false;
    }
  }
  ...
  static async migratePersistedData(
    ...
    try {
      logger.info('Starting migration of persisted data to Supabase...');
      ...
      logger.error('Failed to migrate persisted data:', error);
      return false;
    }
  }
}
