...
async function handleAuthSync(action: any, state: RootState, userId: string) {
  try {
    ...
    logger.error('Failed to sync auth state to Supabase:', error);
  }
}
...
async function handleBucketListSync(action: any, state: RootState, userId: string) {
  try {
        ...
        if (item) {
          ...
          if (venueId) {
            await supabase
              ...
              .eq('user_id', userId)
              .eq('venue_id', venueId);
            logger.info(`[SupabaseMiddleware] Updated notification setting for item ${id} (venue: ${venueId}) to ${enabled}`);
          } else {
            logger.error('[SupabaseMiddleware] Could not find venue_id for item:', item);
          }
        } else {
          logger.error('[SupabaseMiddleware] Could not find item to update notification setting:', id);
        }
        break;
      ...
      case 'bucketList/remove/fulfilled':
        ...
        if (venueId) {
          await supabase
            ...
            .eq('user_id', userId)
            .eq('venue_id', venueId);
          logger.info(`[SupabaseMiddleware] Deleted bucket list item with venue_id: ${venueId}`);
        } else {
          logger.error('[SupabaseMiddleware] Could not find venue_id for item:', action.payload);
        }
        break;
    ...
    logger.error('Failed to sync bucket list to Supabase:', error);
  }
}
...
async function handleUISync(action: any, state: RootState, userId: string) {
  try {
    ...
    logger.error('Failed to sync UI preferences to Supabase:', error);
  }
}
